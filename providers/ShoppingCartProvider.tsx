...
import { InitialStateType, ShoppingCartActionType, ShoppingCartContextType, ShoppingCartProductsType } from '@/types/Providers';
...
const reducer = (state: InitialStateType, action: ShoppingCartActionType) => {
  ...
  switch (action.type) {
    case 'ADD_PRODUCT': {
      ...
      if (state.products.some((product) => product.product.id === action.product.id)) {
        return state;
      }
      // Add new Product
      return {
    ...
    case 'REMOVE_PRODUCT': {
      const productIndex = state.products.findIndex((product) => product.product.id === action.product.id);
      ...
      toast.success(`${product.product.name} removed from cart`, { id: "remove_product" });
      return {
        ...state,
        products: newProducts,
        totalQuantity: state.totalQuantity - product.quantity,
        totalPrice: state.totalPrice - product.quantity * product.product.sell_price,
      };
    }
    ...
    case 'UPDATE_QUANTITY': {
      ...
      // Increment quantity
      if (action.increment) {
        newProducts[productIndex] = { ...product, quantity: product.quantity + 1 };
        return {
          ...state,
          products: newProducts,
          totalQuantity: state.totalQuantity + 1,
